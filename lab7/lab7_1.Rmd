...
msleep %>% 
  filter(bodywt<10) %>% 
  summarise(mean_sleep_smol=mean(sleep_total))
```
...
msleep %>% 
  ...
  summarize(min_monke=min(bodywt),
            mean_monk=mean(bodywt),
            max_monke=max(bodywt)
            )
```
...
```{r}
msleep %>% 
  summarize(n_genera=n_disinct(genus))
```
...
```{r}
msleep %>% 
  summarize(min_mam=min(sleep_total),
            max_man=max(sleep_total),
            mean_mam=mean(sleep_total)
            )
```
...
msleep %>% 
  group_by(order) %>% 
  summarize(brain_wt=mean(brainwt), order, genus)
```
...
msleep %>% 
  ...
  summarize(brain_wt=mean(brainwt), order, genus, na.rm=TRUE)
```
Nobody has measured these brain weights.
```{r}
