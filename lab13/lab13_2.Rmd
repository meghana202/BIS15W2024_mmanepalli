...
Start by making an account with [Stadia Maps](

). You need to add a "property" which is just a name for your project. Once you have an account, you can get an API key. Use the following code to register your key.  
```{r}
...
```{r}
latitude1 <- select(sharks_dups, latitude)
longitude1 <- select(sharks_dups, longitude)
bbox1 <- make_bbox(longitude1, latitude1, f = 0.03) 
```
...
ggmap(map2) + 
  ...
  labs(x= "Longitude", y= "Latitude", title="Sharks attck")
```
...
```{r}
sharks_fatalities <- sharks %>% 
  filter(injury=="fatal") %>% 
  select(latitude, longitude)
```

```{r}
latitude2 <- select(sharks_fatalities, latitude)
longitude2 <- select(sharks_fatalities, longitude)
bbox2 <- make_bbox(longitude2, latitude2, f = 0.03) 
```
...
```{r}
ggmap(map3) + 
  geom_point(data = sharks_fatalities, aes(longitude, latitude), size=0.4) +
  labs(x= "Longitude", y= "Latitude", title="Shark kills")
```
