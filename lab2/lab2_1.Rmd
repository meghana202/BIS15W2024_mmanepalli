...
```{r}
avg_speed <- (venom_GT + chiron + veyron)/3
```
...
mean(cars)
```
The first arg for mean is the object, second is trim (a fraction of trim observations deleted from each end), third is na.rm to strip NA values.  
c stands for concatenate, it creates an object 
```{r}
...
I believe this result means that R cannot calculate the mean due to the NA value. It means that the NA value is not treated as a 0 or any regular value, and cannot be included when calculating mean.  
