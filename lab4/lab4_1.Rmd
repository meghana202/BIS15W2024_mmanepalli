...
```
There are no NA values, but likely someone used negative values to represent them.
```{r}
...
```{r}
mammals <- rename(mammals, genus="Genus", max_life="max. life", litters_per_year="litters/year")
```
