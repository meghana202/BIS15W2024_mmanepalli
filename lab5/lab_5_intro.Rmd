...
Dplyr is a package within the library Tidyverse, it is used to transform data. Select() and filter() are two commands within the package dplyr.
2. Describe how `select ()` can be used to transform data frames.  
Select() allows you to select columns/variables of the data frame.
3. Describe how `filter ()` can be used to transform data frames. 
Filter() can be used to filter() out specific values rather than the variables themselves. 
...
```{r}
BO_fish <- filter(fishlength, lakeid=="BO")
```


4. Calculate the mean length of fish from lake "BO". 
```{r}
mean_BO_fish_length <- mean(BO_fish$length, rm.na=T)
```
