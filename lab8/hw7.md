...
```
No, there are NAs that are not represented by "NA". There are no NA's, but values that represent them. 
...
```
Yes because the NAs are represented as NA. There are many NA's in various columns (counts shown above).
...
amniota %>% 
  ...
  group_by(class) %>% 
  summarize(nas = sum(is.na(egg_mass_g)))
```
...
## #   male_maturity_d <dbl>, inter_litter_or_interbirth_interval_y <dbl>, …
```
There are many NAs, however they do lay eggs and should have a measurable mass. So I believe the NAs were just not able to be measured.
...
```
They aren't NA, but just artifacts of how the data is stored. 1 represents true, and NA probably a false, and not all animals are 'all of the above' for these categories. These are represented as NA. 
