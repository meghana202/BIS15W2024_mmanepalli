...
ui <- dashboardPage(
  ...
  dashboardBody(
    fluidRow(
      ...
      box(width=3,
          selectInput("z", "Select Ethnicity", choices = unique(UC_admit$Ethnicity)), hr()  
      ),
      box(width=7,
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    UC_admit %>% 
      ...
      ggplot(aes(x=Academic_Yr, y=`Perc FR`, fill=Academic_Yr))+
      geom_col() + 
      labs(title="Admissions by Year", x="Year", y="% Admissions") + 
