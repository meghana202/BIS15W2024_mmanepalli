...
ui <- fluidPage(titlePanel("Homerange Locomotion"),
                selectInput("x", "Select Taxon", choices=c("birds", "lake fishes", "lizards", "mammals", "marine fishes", "river fishes", 
...
```{r}
ui <- dashboardPage(
  dashboardHeader(title="oenuings"),
...
```
```{r}
pops <- wolves %>% distinct(pop)
ui <- dashboardPage(
  
  dashboardHeader(title = "Woles"),
  ...
  fluidRow(
    box(width=3,
      selectInput("x", "Select Pop Variable", choices = unique(wolves$pop)), hr() # close the first box   
    ),
