...
```{r}
mammals %>%
  mutate_all(tolower) #made all column names lower case
```
...
mammals %>% 
  select(genus, species, newborn) %>%
  mutate(newborn_new = ifelse(newborn == -999.00, NA, newborn))%>% #ifelse if newborn ==  -999. change to na, if not then no change
  arrange(newborn)
```
...
```
The Balaenoptera physalus has the oldest known lifespan with max life of 1368. It is a fin whale
